...
def sending_mode():
    ...
    if valueCheckbtn_Secure.get():
        ...
        print("\t-> Full Encoded Payload :", codecs.encode(d_payload_encrypted,'hex'))
...
def key_encryption(key, default_static_key):
    ...
    return codecs.encode(cipher.encrypt(default_static_key),'hex')
...
def generate_encrypted_payload(s_node, d_node, payload_to_encrypt):
    ...
    print("\t-> Encoded IV :", codecs.encode(temp_auth,'hex'))
    ...
    l2 = int(codecs.encode(temp_auth,'hex'), 16)
    ...
    print("\t-> Encoded MAC :", codecs.encode(encodedMAC,'hex'))
    ...
    if len(key) == 32:
        ...
        if nonce_device and nonce_other:
            ...
            if 16 < length_encrypted_payload < 32:  # More than 1 block to decrypt
                ...
                try:
                    iv = iv.decode("hex")
                    print("\t\t\t[IV] (hex) : " + codecs.encode(iv,'hex'))

                    cipher = AES.new(key_aes, AES.MODE_OFB, iv)
                    result1 = codecs.encode(cipher.decrypt(payload_enc_block1),'hex')
                    result2 = codecs.encode(cipher.decrypt(payload_enc_block2),'hex')
                ...
                try:
                    ...
                    print("\t\t\t[IV] (hex) : " + codecs.encode(iv,'hex'))
                    cipher = AES.new(key_aes, AES.MODE_OFB, iv)
                    result = codecs.encode(cipher.decrypt(payload_enc),'hex')
                    print("\t\t\t[DECODED] Payload (hex): " + result)
                except:
                    print("Error during decrypting")
    else:
    ...
    if ZwClass in list(zwClasses.ZwaveClass.keys()):
        ...
        if CmdClass in list(zwClasses.ZwaveClass[ZwClass].keys()):
            ...
            if zwClasses.ZwaveClass[ZwClass][CmdClass] == "ManufacturerSpecificCmd_Report":
                ...
                lib = str(int(lib, 16))
                if lib in list(zwClasses.LIBRARY.keys()):
        ...
        if res:
            ...
            for frame in frames:
                ...
                try:
                    ...
                    if calculated_checksum_frame != (fcs):
                ...
                if res:  # if we still have a frame to decode
                    ...
                    #res = codecs.encode(bytes(res), 'hex')
                    res = res.encode().hex()
                    ...
                    try:
                        ...
                        print("decoded_payload not assigned")
